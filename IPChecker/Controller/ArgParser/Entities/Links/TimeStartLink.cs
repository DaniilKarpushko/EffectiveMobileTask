...
public class TimeStartLink : ParserLinkBase
{
    public override ParseResult Handle(IArgIterator iterator, ParsedArgs.Builder argumentCollector)
    {
        if ((string)iterator.Current == "--time-start" && iterator.MoveNext())
        {
            argumentCollector.WithTimeStart((string)iterator.Current);
