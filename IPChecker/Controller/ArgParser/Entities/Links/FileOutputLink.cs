...
public class FileOutputLink : ParserLinkBase
{
    public override ParseResult Handle(IArgIterator iterator, ParsedArgs.Builder argumentCollector)
    {
        if ((string)iterator.Current == "--file-output" && iterator.MoveNext())
        {
            argumentCollector.WithFileOutput((string)iterator.Current);
